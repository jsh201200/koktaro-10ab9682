...
export default function AdminSettings() {
  ...
  // 📥 [연동 핵심] 페이지 접속 시 및 데이터 변경 시 DB와 강제 동기화
  useEffect(() => {
    ...
    const fetchAndSubscribe = async () => {
      ...
      if (data && data.value !== null) {
        const isTest = data.value === true || data.value === 'true' || (typeof data.value === 'object' && data.value.testMode === true);
        setSettings(prev => ({ ...prev, testMode: isTest }));
      }

      // 2. 실시간 구독 (폰이나 다른 기기에서 버튼 바꾸면 같이 움직임)
      const channel = supabase
        .channel('admin-test-mode-realtime')
        .on('postgres_changes', { 
          event: '*', 
          schema: 'public', 
          table: 'site_settings', 
          ...
          if (newVal !== null) {
            const isTest = newVal === true || newVal === 'true' || (typeof newVal === 'object' && newVal.testMode === true);
            setSettings(prev => ({ ...prev, testMode: isTest }));
          }
        })
        .subscribe();
...
const handleSave = async () => {
    ...
    try {
      ...
      // 2. 테스트 모드 상태 DB 저장 (실시간 기기 연동 핵심)
      const { error: testModeError } = await supabase
        .from('site_settings')
        .upsert(
        ...
        >
          ...
          <div className="mb-8 p-4 glass rounded-2xl border border-primary/20 flex justify-between items-center bg-primary/5">
             <div>
               ...
               <p className="text-[10px] text-muted-foreground">켜고 '저장' 누르면 모든 기기 즉시 연동</p>
             </div>
             <button 
               onClick={() => updateField('testMode', !settings.testMode)} 
               className={`relative w-12 h-6 rounded-full transition-colors ${settings.testMode ? 'bg-primary' : 'bg-muted'}`}
             >
            ...
            <div className="space-y-8">
              ...
              <div className="glass-strong rounded-3xl p-6 glow-border mb-8">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button
                    onClick={() => setBannerSettings({ ...bannerSettings, couponActive: !bannerSettings.couponActive })}
                    className={`relative w-12 h-6 rounded-full transition-colors ${bannerSettings.couponActive ? 'bg-primary' : 'bg-muted'}`}
                  >
                ...
                <div className="space-y-4">
                  ...
                  <Field label="배너 텍스트" value={bannerSettings.couponBanner} onChange={(v) => setBannerSettings({ ...bannerSettings, couponBanner: v })} multiline />
                </div>
              </div>
            </div>
          )}
          ...
          {activeTab === 'menus' && (
            <div className="space-y-6">
              ...
              {loadingProducts ? (
                <div className="text-center py-10 opacity-50 text-sm">상품 정보를 불러오는 중...</div>
              ) : (
                      ...
                      {editingProduct?.id === product.id ? (
                        <div className="space-y-3">
                          <div className="grid grid-cols-2 gap-3">
                            <input type="text" value={editingProduct.name} onChange={(e) => setEditingProduct({ ...editingProduct, name: e.target.value })} className="p-2 rounded-lg glass text-sm focus:outline-none" placeholder="메뉴명" />
                            <input type="number" value={editingProduct.price} onChange={(e) => setEditingProduct({ ...editingProduct, price: parseInt(e.target.value) || 0 })} className="p-2 rounded-lg glass text-sm focus:outline-none" placeholder="가격" />
                          </div>
                        ...
                        <div className="flex justify-between items-start">
                          ...
                          <div className="flex gap-2">
                            <button onClick={() => setEditingProduct(product)} className="p-2 rounded-lg glass hover:bg-white/40"><Edit2 className="w-4 h-4 text-primary" /></button>
                            <button onClick={() => handleDeleteProduct(product.id)} className="p-2 rounded-lg glass hover:bg-destructive/10"><Trash2 className="w-4 h-4 text-destructive" /></button>
                          </div>
                        </div>
                      )}
                    </div>
          ...
          {activeTab === 'colors' && (
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <ColorField label="강조색 (Primary)" value={settings.primaryColor} onChange={v => updateField('primaryColor', v)} />
              </div>
            </div>
          )}
          ...
          {activeTab === 'links' && (
            <div className="space-y-5">
              ...
              <Field label="Discord 웹훅" value={settings.discordWebhook} onChange={v => updateField('discordWebhook', v)} />
            </div>
          )}
          ...
          {activeTab === 'legal' && (
            <div className="space-y-5">
              ...
              <Field label="면책 조항" value={settings.disclaimerText} onChange={v => updateField('disclaimerText', v)} multiline />
              <Field label="환불 규정" value={settings.refundPolicy} onChange={v => updateField('refundPolicy', v)} multiline />
            </div>
          )}

          {activeTab === 'messages' && (
            <div className="space-y-5">
              <h2 className="font-serif text-lg font-bold text-secondary-foreground">💬 메시지</h2>
              <Field label="첫 인사 메시지" value={settings.welcomeMessage} onChange={v => updateField('welcomeMessage', v)} multiline />
            </div>
          )}
...
function Field({ label, value, onChange, placeholder, type, multiline }: { label: string; value: string; onChange: (v: string) => void; placeholder?: string; type?: string; multiline?: boolean }) {
  const cls = "w-full p-2.5 rounded-xl glass text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 text-foreground";
  return (
    <div className="space-y-1">
      <label className="text-xs text-muted-foreground font-medium block ml-1">{label}</label>
      {multiline ? (
...
function ColorField({ label, value, onChange }: { label: string; value: string; onChange: (v: string) => void }) {
  return (
    <div className="space-y-1">
      <label className="text-[10px] text-muted-foreground font-medium block ml-1">{label}</label>
      <div className="flex items-center gap-2">
        <input type="color" value={value} onChange={e => onChange(e.target.value)} className="w-9 h-9 rounded-lg border-0 cursor-pointer bg-transparent" />
        <input type="text" value={value} onChange={e => onChange(e.target.value)} className="flex-1 p-2 rounded-xl glass text-xs focus:outline-none font-mono" />
      </div>
    </div>
  );
}
