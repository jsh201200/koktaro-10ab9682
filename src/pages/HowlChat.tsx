...
export default function HowlChat() {
  ...
  const [testMode, setTestMode] = useState(false); // ✨ 실시간 테스트 모드 상태
  ...
  useEffect(() => {
    const fetchTestMode = async () => {
      const { data } = await supabase
        ...
        .eq('key', 'test_mode')
        .single();
      if (data) setTestMode(!!data.value);
    };
    ...
    const channel = supabase
      ...
      .on(
        'postgres_changes',
        { event: 'UPDATE', schema: 'public', table: 'site_settings', filter: 'key=eq.test_mode' },
        (payload) => {
          if (payload.new) setTestMode(!!payload.new.value);
        }
      )
      .subscribe();
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        'postgres_changes',
        { event: 'UPDATE', schema: 'public', table: 'site_settings', filter: `key=eq.coupon` },
        (payload) => {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        'postgres_changes',
        { event: 'UPDATE', schema: 'public', table: 'payments', filter: `session_id=eq.${session.dbSessionId}` },
        async (payload) => {
          ...
          if (updated.status === 'approved') {
            ...
            activatePaidMode(durationMin, updated.menu_id, updated.menu_name || product.name, updated.final_price || updated.price);
          }
        }
      )
      .subscribe();
  ...
  const handleMenuSelect = async (menu: Menu) => {
    ...
    if (menu.id === 0) { setShowPayment(true); return; }
  ...
  const handlePremiumSubmit = async (questions: string[], depositor: string, phoneTail: string) => {
    ...
    sendDiscordAlert({ userName: session.userName || userProfile?.nickname || '', menuName: menu.name, menuId: 16, price, method: 'premium', depositor, phoneTail, questions });
      ...
      <>
        <LandingPage
          ...
          onCheckCredits={() => {
            ...
            else toast.info(`현재 적립금: ${userProfile.credits.toLocaleString()}원`);
          }}
        />
        <button onClick={() => navigate('/admin')} className="fixed top-3 right-3 z-[60] p-2 rounded-full glass hover:bg-muted/60 transition-colors" title="관리자 대시보드">
  ...
  if (view === 'auth') {
    return (
      <div className="min-h-svh aurora-bg">
        <PhoneAuth onAuth={handleAuthComplete} onSkip={() => setView('chat')} />
      </div>
    );
  }
  ...
  const currentCounselor = session.selectedMenu
    ...
    : session.counselorId ? COUNSELORS.find(c => c.id === session.counselorId) || null : null;
  ...
  return (
    <div className="min-h-svh aurora-bg">
      ...
      {session.isPaid && (sessionTime !== null || timerExpired) && (
        <ConsultTimer seconds={sessionTime || 0} expired={timerExpired} onExtend={() => setShowPayment(true)} isAlert={sessionTime !== null && sessionTime <= 300} />
      )}
      ...
      {session.freeReadingDone && !session.isPaid && session.selectedMenu && (
        <div className="fixed bottom-[120px] w-full px-4 z-40">
          <div className="max-w-2xl mx-auto">
            <button onClick={() => { if (session.selectedMenu?.id === 16) setShowPremiumForm(true); else setShowPayment(true); }} className="w-full py-3 rounded-2xl bg-primary text-primary-foreground font-bold text-sm shadow-lg glow-border-hover transition-all active:scale-[0.98] animate-pulse">
      ...
      <AnimatePresence>
        {isMenuOpen && <MenuGrid onSelect={handleMenuSelect} onClose={() => setIsMenuOpen(false)} />}
      </AnimatePresence>
      ...
      {showReview && userProfile && session.dbSessionId && session.selectedMenu && (
        <ReviewModal sessionId={session.dbSessionId} profileId={userProfile.id} userName={userProfile.nickname} menuName={session.selectedMenu.name} paymentPrice={getDbPrice(session.selectedMenu.id)} onClose={() => { setShowReview(false); setShowPremiumReport(true); }} />
      )}
