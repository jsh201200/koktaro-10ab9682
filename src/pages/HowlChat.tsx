...
export default function HowlChat() {
  ...
  const sessionIdRef = useRef(localStorage.getItem('howl_session_id') || `session_${Date.now()}_${Math.random()}`);
  ...
  useEffect(() => {
    const validateSession = async () => {
      ...
      // ✨ [Fix 3] DB에서 roomId, counselorId 등을 정확히 복구
      ...
      if (sessionData) {
        // 🔧 메뉴 복구 (dbProducts 로딩 전에도 MENUS에서 기본값 사용)
  ...
  // 🔧 [Fix 2] DB 가격 + MENUS 폴백
  const getDbPrice = (menuId: number): number => {
    const product = dbProducts.find(p => p.menu_id === menuId);
    if (product?.price) return product.price;

    // DB 로딩 실패 시 MENUS 기본 가격으로 폴백
  ...
  const handleBotResponse = useCallback(async (
    ...
    try {
      ...
      // counselorId가 없으면 session에서 가져옴
  ...
  }, [messages, session.counselorId, session.selectedMenu, session.roomId, addBotMessage, setIsTyping, delayedTyping]);

  // 크로스셀링 (placeholder - 원본에서 가져와야 함)
  const handleCrossSelling = useCallback(() => {
    addBotMessage("다른 메뉴도 확인해볼래? 🔮 아래 '메뉴 보기'를 눌러봐!");
  }, [addBotMessage]);

  const handleMenuSelect = async (menu: Menu) => {
    setIsMenuOpen(false);

    // 1️⃣ 클릭한 메뉴에 맞는 상담사를 새로 데려옴
    const counselor = getCounselorForMenu(menu.id);
    // 2️⃣ 상담사 ID가 포함된 고유한 방 번호
  ...
  const handleSend = async (text: string, image?: string) => {
    ...
    // session.counselorId를 사용하거나, 선택된 메뉴로부터 상담사를 결정
  ...
  // ✨ 로그인 상태를 확인해서 메인으로 보낼지, 로그인창으로 보낼지 결정
    ...
    <div className="flex flex-col h-screen max-w-lg mx-auto relative">
      ...
      {showExitModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          ...
          <div className="relative z-10 bg-background rounded-3xl p-6 mx-4 max-w-sm w-full shadow-xl">
            <h3 className="text-lg font-bold text-center mb-2">상담을 종료할까요?</h3>
            <p className="text-sm text-muted-foreground text-center mb-4">대화 내용을 어떻게 할까요?</p>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        <AnimatePresence>
          {messages.map((msg) => (
            <MessageBubble key={msg.id} message={msg} counselor={currentCounselor} />
          ))}
        </AnimatePresence>
      ...
      {showPayment && session.selectedMenu && (
        <PaymentModal
          ...
          price={getDbPrice(session.selectedMenu.id)}
