...
// ✨ [Fix #2] 결제금액 0원 버그 방지용 Fallback 가격
...
export default function HowlChat() {
  ...
  // ✨ [Fix #2] testMode 실시간 구독 - 모바일 ↔ 컴퓨터 동기화
  useEffect(() => {
    const fetchTestMode = async () => {
      ...
      if (data && data.value !== null) {
        ...
        setTestMode(isTest);
        console.log('🧪 testMode 로드됨:', isTest);
      }
    };
      ...
      .on(
        ...
        (payload) => {
          ...
          if (payload.new && payload.new.value !== null) {
            ...
            setTestMode(isTest);
            console.log('✅ testMode 동기화됨:', isTest);
            toast.info(`테스트 모드: ${isTest ? 'ON' : 'OFF'}`);
          }
        }
      )
      .subscribe();
  ...
  // ✨ [Fix #2] 결제금액 0원 버그 해결 - Fallback 로직 추가
  const getDbPrice = (menuId: number): number => {
    ...
    // DB가 로드 안 됐거나 가격이 0이면 Fallback 사용
    const fallbackPrice = MENU_FALLBACK_PRICES[menuId];
    if (fallbackPrice) {
      console.warn(`⚠️ [Fix #2] DB 가격 없음, Fallback 사용: ${menuId} → ${fallbackPrice}원`);
      return fallbackPrice;
    }
    
    // 최후의 수단: 기본값 9900
    console.error(`❌ [Fix #2] 메뉴 ${menuId}의 가격을 찾을 수 없음, 기본값 9900 사용`);
    return 9900;
  };
 ...
 const handleCrossSelling = useCallback(() => {
  // 1. 현재 세션에 저장된 상담사 ID를 최우선으로 가져옵니다.
  const currentId = session.counselorId || (session.selectedMenu ? getCounselorForMenu(session.selectedMenu.id).id : 'ian');
...
}, [session.counselorId, session.selectedMenu, addBotMessage]);
