    ...
    <header className="fixed top-0 w-full z-50 glass px-4 py-3 sm:px-6">
      <div className="max-w-2xl mx-auto flex justify-between items-center">
        <div className="flex items-center gap-3">
          {onBack && (
            <button onClick={onBack} className="p-1 rounded-lg hover:bg-white/50 transition-colors">
          ...
          <div>
            <h1 className="font-serif text-lg font-bold tracking-tight text-secondary-foreground">
              {counselorName ? `${counselorName}` : '하울의 챗봇상담소'}
            </h1>
            <p className="text-[10px] text-muted-foreground">
              {counselorName ? '상담 진행 중' : '천상계 점술 상담'}
            </p>
          </div>
        </div>
